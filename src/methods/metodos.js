...
export class Objetos{
  ...
  async carregarCidades(arquivoPath = null) {
    ...
    let raw;
    try {
      raw = await fs.readFile(arquivo, "utf8");
    } catch (err) {
      throw new Error(
  ...
  async buscandoPopulacao(cidade, uf, options = {}) {
    ...
    const match = lista.find((row) => {
      const rowCidade = Objetos.#normalizeText(row?.Cidade ?? row?.cidade);
      const rowUf = Objetos.#normalizeUF(row?.uf ?? row?.UF);
